...
def drone_test_publisher():
    ...
    while not rospy.is_shutdown():
        if (rospy.Time.now()-flight_time)<=rospy.Duration(wait_time):
            ...
            drone_vel.angular.z = 0
            print("pause")
        elif (rospy.Time.now()-flight_time)<=rospy.Duration(wait_time+pause_time*5+run_time*5):
            drone_vel.linear.x = 1
            drone_vel.linear.y = 0
            drone_vel.linear.z = 0#[0.05,0.05,0.05]
            drone_vel.angular.z = 0
            print("east")
        elif (rospy.Time.now()-flight_time)<=rospy.Duration(wait_time+pause_time*6+run_time*5):
            drone_vel.linear.x = 0
            drone_vel.linear.y = 0
            drone_vel.linear.z = 0#[0.05,0.05,0.05]
            drone_vel.angular.z = 0
