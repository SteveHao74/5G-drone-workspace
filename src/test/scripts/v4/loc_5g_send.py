...
def tcp_link(odom_msg, col_img_msg, dep_img_msg):
    ...
    ret, jpg1 = cv2.imencode('.jpg', col_img, encode_param)
    len1 = str(len(jpg1)).zfill(7)
    # ret, jpg2 = cv2.imencode('.png', dep_img)
    dep_img_1 = np.uint8(dep_img & 0x00ff)
    dep_img_2 = np.uint8(dep_img >> 8)
    ret, jpg2 = cv2.imencode('.jpg', dep_img_1, encode_param)
    len2 = str(len(jpg2)).zfill(7)
    ret, jpg3 = cv2.imencode('.jpg', dep_img_2, encode_param)
    len3 = str(len(jpg3)).zfill(7)
