...
def callback_syn(ros_gps_pos,ros_gps_vel,ros_odom,ros_local_vel):
    global ifm
    print("1234")
    ...
    gps_imu.imu.quan_x = ros_odom.pose.pose.orientation.x
    gps_imu.imu.quan_y = ros_odom.pose.pose.orientation.y
    gps_imu.imu.quan_z = ros_odom.pose.pose.orientation.z
    gps_imu.imu.quan_w = ros_odom.pose.pose.orientation.w
    
    gps_imu.imu.w_x = 0#ros_imu.angular_velocity.x
    gps_imu.imu.w_y = 0#ros_imu.angular_velocity.y
    gps_imu.imu.w_z = 0#ros_imu.angular_velocity.z
    sent_data = gps_imu.SerializeToString()
    # drone_syn_pub.publish(drone_syn)
    ifm.send_state(sent_data)
    print("finish",ros_odom.pose.pose.orientation.x)
...
if __name__ == '__main__':
    ...
    sub_local_odom = message_filters.Subscriber('/mavros/local_position/odom', Odometry)
    sub_imu = message_filters.Subscriber('/mavros/imu/data', Imu)
    sync_listener = message_filters.ApproximateTimeSynchronizer([sub_gps,sub_gps_vel,sub_local_odom,sub_local_vel], 10, 1)#, allow_headerless=True
