...
def talker():
    ...
    while not rospy.is_shutdown():
        ...
        if len(data) != img_len:
            continue
        col_img = cv2.imdecode(np.frombuffer(data[:len1], dtype='uint8'), cv2.IMREAD_COLOR)
        dep_img_1 = cv2.imdecode(np.frombuffer(data[len1:len1 + len2], dtype='uint8'), cv2.IMREAD_ANYDEPTH)
        dep_img_2 = cv2.imdecode(np.frombuffer(data[len1 + len2:], dtype='uint8'), cv2.IMREAD_ANYDEPTH)
